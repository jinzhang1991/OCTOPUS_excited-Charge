...
contains
  ...
  subroutine td_run(sys, fromScratch)
    ...
    type(restart_t)              :: restart_load, restart_dump, restart
    ...
    call td_write_init(write_handler, sys%namespace, sys%outp, gr, sys%st, sys%hm, geo, sys%ks, &
    ...
    propagation: do iter = td%iter, td%max_iter
       ...
       !calculate n_ex
       call td_write_iter(write_handler, sys%namespace, sys%outp, gr, st, sys%hm, geo, sys%hm%ep%kick, td%dt, iter, sys%mc)
  ...
  contains
    ...
    subroutine check_point()
      ...
      if((sys%outp%output_interval > 0 .and. mod(iter, sys%outp%output_interval) == 0) .or. &
        ...
        !jzhang
        call td_write_output(sys%namespace, gr, sys%st, sys%hm, sys%ks, sys%outp, geo, iter, td%dt)
      end if
      ...
      if (mod(iter, sys%outp%restart_write_interval) == 0 .or. iter == td%max_iter .or. stopping) then ! restart
        ...
        if (ion_dynamics_ions_move(td%ions) .and. td%recalculate_gs) then
          ...
          call messages_print_stress(stdout, "Time-dependent simulation proceeds jz 2021!", namespace=sys%namespace)
          call print_header(sys%namespace)
        end if
      end if
    ...
    subroutine td_run_zero_iter()
      ...
      call td_write_iter(write_handler, sys%namespace, sys%outp, gr, st, sys%hm, geo, sys%hm%ep%kick, td%dt, 0, sys%mc)
